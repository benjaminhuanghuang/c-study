    ...
    */
    ...
    while(i< source_office->packages_count){
        if(source_office->packages[i].weight >= target_office->min_weight && source_office->packages[i].weight <= target_office->max_weight){
            ...
            memcpy(target_office->packages+target_office->packages_count-1, source_office->packages+i, sizeof(package));
            memcpy(source_office->packages+i, source_office->packages+i+1, (source_office->packages_count-i)*sizeof(package));
        }
        else
        {
            i++;
...
int count_package(town t){
    int p =0;
    for(int i =0; i < t.offices_count; i++){
       p += t.offices[i].packages_count;
    }
...
town town_with_most_packages(town* towns, int towns_count) {
    ...
    for(int i=0; i < towns_count; i++){
        int pcount = count_package(towns[i]);
        if(pcount > most_package){
            most_package = pcount;
            index = i;
        }
    }
